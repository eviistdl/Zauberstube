...
// Funktion, um die sichtbaren Bilder aus einem Array zu erhalten
function getVisibleBilderFromArray(bilderArray) {
  return bilderArray.filter(bild => bild.style.visibility === "visible");
}

function getVisibleBilder() {
  var visibleBilder = [];
  visibleBilder.push(getVisibleBilderFromArray(bilderRezept0));
  visibleBilder.push(getVisibleBilderFromArray(bilderRezept1));
  visibleBilder.push(getVisibleBilderFromArray(bilderRezept2));
  return visibleBilder;
}
...
document.getElementById("fertig_button").onclick = function () {
  ...
  //resetBilder();
  // Reset des ImageSwitcher-Zustands
  ing1_chooser.Reset();
  ing2_chooser.Reset();
  Seal_chooser.Reset();

  // Übergeben der aktuellen Daten an die checkRezept-Funktion
  checkRezept(getVisibleBilder());
};
...
/*function resetBilder() {
  console.log("resetBilder wurde aufgerufen");
...
/*window.onload = function () {
  ...
  // Hier die ImageSwitcher erstellen, nachdem die Seite vollständig geladen wurde
...
// Erweitere die ImageSwitcher-Funktion um die Reset-Methode
...
function checkRezept(visibleBilder) {
  // Überprüfen, welches Rezept aktuell angezeigt wird
  var currentRezept = RezeptArray[Index];

  // Je nach Rezept das entsprechende Bild-Array abrufen
  var correctBilder = [];
  if (currentRezept === RezeptArray[0]) {
      correctBilder = [zahn, feder, wachs];
      console.log("Rezept0 wird geprüft")
  } else if (currentRezept === RezeptArray[1]) {
      correctBilder = [kristal, bluete, tuch];
  } else if (currentRezept === RezeptArray[2]) {
      correctBilder = [kralle, haar, korken];
  }

  if(correctBilder.lenghth > 0){
